  ...
  return (
    <section id="ranking" className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="grid gap-6 lg:grid-cols-[1.02fr_0.98fr]">
        <div className="rounded-[32px] bg-white p-6 shadow-[0_16px_50px_rgba(15,23,42,0.06)] sm:p-8">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-black tracking-[-0.03em] text-[#111827]">오늘 뭐볼까? 지금 HOT한 공연</h2>
            </div>
            <span className="rounded-full bg-[#eef1ff] px-3 py-1 text-xs font-bold text-[#4154ff]">실시간 반영</span>
          </div>
          ...
          <div className="mt-7 space-y-4">
            {rankingCards.map((card) => (
              <article key={card.rank} className="flex gap-4 rounded-[24px] bg-[#f8f8fc] p-4 ring-1 ring-black/3">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#4154ff] text-lg font-black text-white">
                  {card.rank}
                </div>
        ...
        <div className="rounded-[32px] border border-dashed border-[#cfd8ff] bg-[#f6f8ff] p-6 sm:p-8" id="parity">
